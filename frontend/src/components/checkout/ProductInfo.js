    ...
    return (
        <div className="col-md-6">
            ...
            {displayProducts.map((item, index) => {
                const productName = item.productId?.data?.name || item.name;
                const productImage = item.productId?.data?.image || item.image;
                const productPrice = item.productId?.data?.price || item.price;
                const discount = item.productId?.data?.discount || item.discount;
