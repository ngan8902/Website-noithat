  ...
  const FaceRegistration = () => {
    ...
    const saveFace = useCallback(async () => {
      console.log("Hàm saveFace được gọi");
      console.log("staffId trước khi tạo formData:", staffId);
      ...
      <div className="app-container">
        <header className="app-header">
          ...
          {!capturedImage && (
            <button
              onClick={capture}
              disabled={isCapturing || !modelLoaded}
              className="capture-button"
            >
              {isCapturing ? "Đang chụp..." : modelLoaded ? "Chụp Ảnh" : "Đang tải..."}
            </button>
          )}
