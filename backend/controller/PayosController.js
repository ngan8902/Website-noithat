...
const initiatePayment = async (req, res) => {
    try {
        const { amount, description, orderCode, buyerName, buyerPhone, buyerAddress, items, tempOrderData } = req.body;

        if (!amount || !description || !orderCode || !tempOrderData) {
            return res.status(400).json({ message: "Thiếu thông tin thanh toán!" });
        }

        const paymentLink = await payosService.createPayment(amount, description, orderCode, buyerName, buyerPhone, buyerAddress, items);
